// Define an arrow function that builds the HTML table
...
function getTableData() {
	...
	d3.json(url).then(function(crimeData, err){
		if (err) throw err;
	
		// parse data
		// date,primary_type,crimes_committed
		crimeData.forEach(function(sqldata) {
			//sqldata.date = parseTime(sqldata.date),
			sqldata.date = sqldata.date;
			sqldata.primary_type = sqldata.primary_type;
			sqldata.crimes_committed = +sqldata.crimes_committed;
		});
